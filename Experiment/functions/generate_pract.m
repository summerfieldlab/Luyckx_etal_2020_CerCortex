function [ data ] = generate_pract(notrials)
...
data.seq=[ones(1,notrials/8) 1*ones(1,notrials/8)];
...
for i=1:notrials;
    ...
    if data.type(i)<3
        ...
        if data.type(i)==3
            df=15+rand*10;
            data.difs(i,1:ll/3)=[df*ones(1,ll/3)];
            A=[];
            B=[];
            for j=1:ll/3
                b=data.bases(i,j);
                difsA=[0 data.difs(i,j) 2*data.difs(i,j)];
...
% select number of trials for practice block
data.left= data.left(1:6,:);
data.dd=data.dd(1:6);
data.right=data.right(1:6,:);
data.Lm=data.Lm(1:6);
data.Rm=data.Rm(1:6);
data.Lv=data.Lv(1:6);
data.Rv=data.Rv(1:6);
data.mean=data.mean(1:6,:);
data.order=data.order(1:6,:);
data.side=data.side(1:6);
data.block=0*data.mean;
data.sed=sed;
data.type=data.type(1:6,:);
data.randgap=data.randgap(1:6,:);
